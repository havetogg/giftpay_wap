...
function initAllOilRecord() {
    if (isData) {
        $.ajax({
            type: "GET",
            url: getRootPath() + "/count/countOilRecordAllInfo.htm",
            dataType: "json",
                ...
                if (data.length > 0) {
                    ...
                    $.each(data, function (index, item) {
                        if (index >= pageNum && index < (pageNum * 1 +5)) {
                            ...
                            str += '</h1><p><label>金额：</label>';
                            str += '<span class="f40">' + item.payamount + '</span>元';
                            str += '</p><p><label>期数：</label>';
                            str += '<span>' + item.cycle + '期</span>';
                            str += '</p><p>';
                            str += '<label>购买日期：</label>';
                            str += '<span>' + item.createtime.substring(0, item.createtime.indexOf('.')) + '</span>';
                            str += '</p><b>' + (orderNo) + '</b></li>';
                        }
                        money += item.payamount * 1;
                    })
                } else {
                    isData = false;
                }
                $(".redPacket_list ul").append(str);
                $(".allcount").text(data.length-2);
                $(".allmoney").text(money-(2850*2));
            },
            error: function (res) {
                console.log(res)
            }
