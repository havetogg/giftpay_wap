...
<head>
    ...
    <script>
        ...
        function toSellCar() {
        window.location.href='./carValuation.jsp';
        }
        function toBack(){
            history.go(-1);
        }
...
<div style="background-color: #f0f0f4;overflow-y: scroll;" class="zoomer">
 ...
 <div>
    ...
    <div class="jmt_center">
        <img src="img/sellCar_banner.png" alt="" onclick="toBack()" style="margin-top: 50px;">
    </div>
 </div>
    <!--品牌-->
...
//                        doc.getElementById('s').innerText=(items[0] || {}).text;
        ...
        function startCul() {
            var car=$('#userCar').val();
            var model=$('#userCar').attr("data-modelid");//model车型标识
            var brand=$('#userCar').attr("data-brandid");//车300品牌ID
            console.log("品牌："+brand);
            var series=$('#userCar').attr("data-seriesid");//车300车系ID
            var city = "";
            ...
            else if(!jxTool.isMobile(tel)&&(tel.length==11)){
                TipShow('请输入正确的手机号',1000);
            }
            else{
                 loading("start");
            	 $.ajax({
                 	 url: getRootPath() + "/wechat/carValuation/sell.htm",
                 	 data:{"model":model,"regDate":date,"tel":tel,"city":city,"brand":brand,"series":series,"status":"1"},
                 	 dataType:"json",
                 	 type:"post",
                 	 success:function(data){
                 		 console.log(data);
