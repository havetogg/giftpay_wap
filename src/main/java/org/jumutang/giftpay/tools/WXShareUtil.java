...
/**
 * 微信分享工具 Created by liuchao on 2015/1/22.
 */
public class WXShareUtil {

	private static Map<String, String> code_info = null;

	private static String gateUrl="www.linkgift.cn";
	...
	public static WXShareUtil getInstance(String appId, String appSceret) {
		if (null == instance)
			instance = new WXShareUtil(appId, appSceret);
		return instance;
	}
	...
	public String genJSSDKConf(String currUrl) throws Exception {
		...
		this.getJsapiTicket();
		String signature = this.SHA1(String.format("jsapi_ticket=%s&noncestr=%s&timestamp=%d&url=%s", jsapi_ticket,
				nonceStr, timestamp, currUrl));
	...
	private String genNonceStr() {
		...
		for (int i = 0; i < 16; i++) {
			...
			switch (type) {
			case 0:
				sbNonceStr.append((char) (random.nextInt(10) + 48));
				break;
			case 1:
				sbNonceStr.append((char) (random.nextInt(26) + 65));
				break;
			case 2:
				sbNonceStr.append((char) (random.nextInt(26) + 97));
				break;
			}
		}
	...
	private void getAccessTocken() throws Exception {
		...
		String signature = MD5X.getLowerCaseMD5For32(this.appId + this.appSecret + timestamp);
		String url = "http://"+gateUrl+"/gift_gate/getToken.action?appId=" + this.appId + "&timestamp=" + timestamp
				+ "&signature=" + signature;
		System.out.println("access_token_request_url = " + url);
		JSONObject jsonObject = JSONObject.fromObject(postOrGet.sendPost(url, null));
	...
	private void getJsapiTicket() throws Exception {
		...
		timestamp = System.currentTimeMillis() / 1000;
		String signature = MD5X.getLowerCaseMD5For32(this.appId + this.appSecret + timestamp);
		String url = "http://"+gateUrl+"/gift_gate/getJsApiTicket.action?appId=" + this.appId + "&timestamp=" + timestamp
				+ "&signature=" + signature;
		System.out.println("jsapi_ticket_request_url = " + url);
		JSONObject jsonObject = JSONObject.fromObject(postOrGet.sendPost(url, null));
